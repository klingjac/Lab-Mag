...
class IntEnum(int):
    def __new__(cls, value, name=None):
        ...
        obj._name_ = name if name else str(value)
        return obj
    ...
    @classmethod
    def values(cls):
        return list(cls.members().values())

    @classmethod
    def names(cls):
        return list(cls.members().keys())
...
class PniRm3100:
    ...
    def write_tmrc(self):
        # TMRC
        print(f"first: {self.device_addr}, second:{self.TmrcRegister.TMRC_REGISTER_ADDR}, third:{self.tmrc_byte}")
    ...
    def read_meas_x(self):
        ...
        return x_mag_value, x_mag_bytes
    ...
    def read_meas_y(self):
        ...
        return y_mag_value, y_mag_bytes
    ...
    def read_meas_z(self):
        ...
        return z_mag_value, z_mag_bytes

    """
    read_meas() reads X-,Y-,Z-axis magnetometers and returns a list of the values in microtesla (uT) [x,y,z]
    """
    def read_meas(self):
        x_mag_value, x_bytes = self.read_meas_x()
        y_mag_value, y_bytes = self.read_meas_y()
        z_mag_value, z_bytes = self.read_meas_z()
        ...
        return [x_mag_value, y_mag_value, z_mag_value, x_bytes, y_bytes, z_bytes]
        print("X bytes: ", x_bytes)
        print("Y bytes: ", y_bytes)
        print("Z bytes: ", z_bytes)
